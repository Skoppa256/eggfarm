...
import { submitGradingAction } from "@/app/(app)/grading/actions";
...
async function setup() {
  ...
  await createCollection(
    { farmhouseId: farmhouse.id, date: DATE, batchNumber: 1 },
    { goodEggs: 3000, telurRetak: 0, telurLunak: 0, telurKosong: 0, lifts: [] },
    { userId: setupUser.id },
  );
...
describe("submitGradingAction (rule 5.5)", () => {
  it("rejects an OWNER", async () => {
    ...
    await expect(
      submitGradingAction(
        null,
        form({ farmhouseId, date: "2026-07-01", batchNumber: "1", [`q:${normal}:A`]: "10" }),
      ),
    ).rejects.toBeInstanceOf(ForbiddenError);
  });

  it("lets an ADMIN submit grading, converting rak → pcs and posting per SKU", async () => {
    const { whA, farmhouseId, normal } = await setup();
    await loginAs(Role.ADMIN);

    const result = await submitGradingAction(
      null,
      form({ farmhouseId, date: "2026-07-01", batchNumber: "1", [`q:${normal}:A`]: "10" }), // 10 rak → 300 pcs
    );
